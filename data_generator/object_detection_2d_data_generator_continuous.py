...
class DataGeneratorContinuous:
    '''
    This one is adapted to the specific case of one class and one clabel.
    ...
    def __init__(self,
                 ...
                 image_ids=None,
                 eval_neutral=None,
                 labels_output_format=('class_id', 'clabel_value', 'xmin', 'ymin', 'xmax', 'ymax'),
                 verbose=True):
        '''
        ...
        Arguments:
            ...
            labels_output_format (list, optional): A list of five strings representing the desired order of the six
                items class ID, clabel, xmin, ymin, xmax, ymax in the generated ground truth data (if any). The expected
                strings are 'xmin', 'ymin', 'xmax', 'ymax', 'class_id', 'clabel_value'.
            verbose (bool, optional): If `True`, prints out the progress for some constructor operations that may
                take a bit longer.
        '''
        self.labels_output_format = labels_output_format
        self.labels_format={'class_id': labels_output_format.index('class_id'),
                            'clabel_value': labels_output_format.index('clabel_value'),
    ...
    def parse_xml(self,
                  ...
                  verbose=True):
        '''
        Adapted version to handle continuous labels. This version can only handle one class, clabels are defined by clabel_map.
        Classes in xml files that are in the clabel_map are used, classes not in clabel_map are ignored.
        ...
        Arguments:
            ...
            image_set_filenames (list): A list of strings, where each string is the path of the text file with the image
                ...
                and nothing else.
            clabel_map (dict): A dictionary containing a mapping of class names to float values. Classes not defined in
            this dictionary will be ignored.
            ...
            for image_id in it:
                ...
                if not annotations_dir is None:
                    ...
                    for obj in objects:
                        ...
                        if not class_name in self.clabel_map: continue
                        clabel_value = self.clabel_map[class_name]
                        ...
                        item_dict = {'folder': folder,
                                     ...
                                     'class_id': 1, # Always 1, because in this implementation there is only one class (0 is background)
                                     'clabel_value': clabel_value,
                                     'pose': pose,
                                     'truncated': truncated,
                                     'difficult': difficult,
                                     'xmin': xmin,
    ...
    def generate(self,
        ...
        Arguments:
            ...
            returns (set, optional): A set of strings that determines what outputs the generator yields. The generator's output
                ...
                * 'matched_anchors': Only available if `labels_encoder` is an `SSDInputEncoderContinuous` object. The same as 'encoded_labels',
        ...
        elif not isinstance(label_encoder, SSDInputEncoderContinuous):
            if 'matched_anchors' in returns:
                warnings.warn("`label_encoder` is not an `SSDInputEncoderContinuous` object, therefore 'matched_anchors' is not a possible return, " +
                              "but you set `returns = {}`. The impossible returns will be `None`.".format(returns))
        ...
        while True:
            ...
            if not (label_encoder is None or self.labels is None):
                ...
                if ('matched_anchors' in returns) and isinstance(label_encoder, SSDInputEncoderContinuous):
