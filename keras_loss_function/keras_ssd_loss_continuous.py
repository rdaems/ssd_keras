'''
Adaptation for continous labels.
...
class SSDLossContinuous:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        '''
        n_classes = 2   # fixed now, could be dynamic in the future
        ...
        # 2b: Compute clabel loss for positive targets.

        pos_clabel_loss = tf.reduce_sum(clabel_loss * positives, axis=-1)
        ...
        total_loss = (class_loss + self.beta * pos_clabel_loss + self.alpha * loc_loss) / tf.maximum(1.0, n_positive) # In case `n_positive == 0`
