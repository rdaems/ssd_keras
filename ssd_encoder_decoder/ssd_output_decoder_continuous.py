...
def decode_detections(y_pred,
    ...
    Arguments:
        y_pred (array): The prediction output of the SSD model, expected to be a Numpy array
            of shape `(batch_size, #boxes, #classes + #clabels + 4 + 4 + 4)`, where `#boxes` is the total number of
            boxes predicted by the model per image and the last axis contains
            `[one-hot vector for the classes, clabels vector, 4 predicted coordinate offsets, 4 anchor box coordinates, 4 variances]`.
    ...
    Returns:
        ...
        for one image and contains a Numpy array of shape `(boxes, 6 + n_clabels)` where each row is a box prediction for
        a non-background class for the respective image in the format `[class_id, clabel vector, confidence, xmin, ymin, xmax, ymax]`.
    '''
    n_classes = 2 # fixed now, could be added as argument
    if normalize_coords and ((img_height is None) or (img_width is None)):
        raise ValueError("If relative box coordinates are supposed to be converted to absolute coordinates, the decoder needs the image size in order to decode the predictions, but `img_height == {}` and `img_width == {}`".format(img_height, img_width))

    # 1: Convert the box coordinates from the predicted anchor box offsets to predicted absolute coordinates

    y_pred_decoded_raw = np.copy(y_pred[:,:,:-8]) # Slice out the classes, clabels and the four offsets, throw away the anchor coordinates and variances, resulting in a tensor of shape `[batch, n_boxes, n_classes + n_clabels + 4 coordinates]`
    ...
    n_clabels = y_pred_decoded_raw.shape[-1] - 4 - n_classes # The number of clabels is the length of the last axis minus the four box coordinates and minus the number of classes

    y_pred_decoded = [] # Store the final predictions in this list
    for batch_item in y_pred_decoded_raw: # `batch_item` has shape `[n_boxes, n_classes + n_clabels + 4 coords]`
        pred = [] # Store the final predictions for this batch item here
        for class_id in range(1, n_classes): # For each class except the background class (which has class ID 0)...
            single_class_indices = [class_id] + list(range(n_classes, n_classes + n_clabels + 4))
            single_class = batch_item[:,single_class_indices] # ...keep only the confidences for that class, making this an array of shape `[n_boxes, 1 + n_clabels + 4]` and...
            ...
            if threshold_met.shape[0] > 0: # If any boxes made the threshold...
                ...
                maxima_output = np.zeros((maxima.shape[0], maxima.shape[1] + 1)) # Expand the last dimension by one element to have room for the class ID. This is now an arrray of shape `[n_boxes, 2 + n_clabels + 4]`
