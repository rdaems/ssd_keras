'''
Adaptation of original file to work with continuous labels1.
...
class SSDInputEncoderContinuous:
    '''
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        Arguments:
            ground_truth_labels (list): A python list of length `batch_size` that contains one 2D Numpy array
                ...
                `(class_id, clabel_0, ..., clabel_(n-1), xmin, ymin, xmax, ymax)` (i.e. the 'corners' coordinate format), and `class_id` must be
        ...
        Returns:
            `y_encoded`, a 3D numpy array of shape `(batch_size, #boxes, #classes + #clabels + 4 + 4 + 4)` that serves as the
        ...
        for i in range(batch_size): # For each batch item...
            ...
            labels_one_hot = np.concatenate([classes_one_hot, labels[:, clabel_id:xmin] ,labels[:, [xmin,ymin,xmax,ymax]]], axis=-1) # The one-hot version of the labels for this batch item
    ...
    def generate_encoding_template(self, batch_size, diagnostics=False):
        ...
        # 3b: Create a template tensor to hold continuous labels
        clabels_tensor = np.zeros((batch_size, boxes_tensor.shape[1], self.n_clabels))
        ...
        y_encoding_template = np.concatenate((classes_tensor, clabels_tensor, boxes_tensor, boxes_tensor, variances_tensor), axis=2)
